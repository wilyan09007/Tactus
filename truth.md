...
- **The UI is [`web/glow.html`](web/glow.html)** (serve `web/`; `/` redirects to it). The live fret-glow LEARN view: per-string 6-hue markers annotated **on the real neck** over the webcam (Sherbet theme), ADSR glow envelope, and alphaTab real tab on the left synced **silent** (render-only, no synth — correct for a Deaf-focused app) from one `SONG` source that also feeds the glow. **The camera is always on and the grid auto-locks MARKERLESS** — `web/fretboard-autodetect.js` (OpenCV.js, an in-browser port of the §3.8/§6 fret-law homography in `software/ai/vision/fretboard_detect.py`) finds the fretboard's 4 corners on the live feed and snaps the grid onto the real neck (the 12-TET law lays the frets inside the quad; corners smoothed for tracking) — **no manual calibration**. `web/aruco-poselock.js` (js-aruco2 marker) remains an optional robustness/validation path. Earlier prototypes are archived in `web/_prototypes/`, theme drafts in `web/_themes/` — **not the product**.
