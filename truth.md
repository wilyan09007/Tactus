...
BROWSER (one tab)                          PYTHON process (laptop)
 ...
 ├ markerless fretboard homography(fret-law)├ FUSION model (position + buzz inverse + Bayesian)
...
- **The UI is [`web/glow.html`](web/glow.html)** (serve `web/`; `/` redirects to it). The live fret-glow LEARN view: per-string 6-hue markers annotated **on the real neck** over the webcam (Sherbet theme), ADSR glow envelope, and alphaTab real tab on the left synced **silent** (render-only, no synth — correct for a Deaf-focused app) from one `SONG` source that also feeds the glow. **The camera is always on** and the grid maps onto the real neck two ways (the 12-TET law lays the frets inside the 4-corner quad): **(1) AUTO** — a local Python backend [`software/ai/vision/fretboard_server.py`](software/ai/vision/fretboard_server.py): Ultralytics **YOLO-World** (open-vocab "guitar" detect) → **SAM2** mask → distance-transform **neck isolation** (separates the thin fretboard from the round body) → the neck quad, on the M4 Pro GPU (MPS), streamed to glow.html over a WebSocket (`ws://localhost:8770`). Robust to busy backgrounds (where edge/hand methods failed). ~1–4 s/frame on MPS → it re-locks every few seconds (the browser self-paces, one frame in flight); good for a held guitar; a **"⇄ flip"** button fixes orientation. **(2) MANUAL fallback** — drag the 4 corner dots onto the neck once, saved to localStorage (exact, bulletproof on any background). A **"✋ auto-fit / ✥ drag mode"** toggle switches them. **Confirmed dead ends** (kept in-repo, NOT loaded by the UI): `web/fretboard-autodetect.js` (OpenCV/Hough — found zero neck edges on a busy hackathon background) and `web/hand-anchor.js` (MediaPipe hand-anchor — a hand alone can't determine the fretboard quad → degenerate). `web/aruco-poselock.js` (marker) = optional. Run the backend: `software/ai/vision/.venv-yolo/bin/python software/ai/vision/fretboard_server.py`. Earlier prototypes are archived in `web/_prototypes/`, theme drafts in `web/_themes/` — **not the product**.
- **Haptic output engine (BUILT — `software/haptic/`, `docs/28`).** The 12-ch haptic out is a **low-level per-speaker control API**, deliberately split from perception: *which* speaker fires for a played note is the **ML/fusion** decision (§6); *play speaker N now with this spec, non-blocking, while others play* is the deterministic **engine** (`HapticEngine.play(ch, …)`). The §5 strum/chord/sustain *behaviours* are composed **on top** by the encoder making many `play()` calls. **Decision (locked): in-process Python API** (the ML runs in this process → lowest latency), not a network endpoint; the `drive[]` WebSocket telemetry rides a `drive_state()` hook (later PR). Output is **cross-platform** (Win WDM-KS / mac CoreAudio / Linux ALSA) via **persistent non-blocking `OutputStream`s — one per Vantec** (or one mac Aggregate stream) feeding a real-time voice mixer — this **supersedes the Linux-only "ALSA → Vantecs" sketch above** and the old blocking one-at-a-time bring-up playback. At `start()` the engine tries an **ordered list of output options** (discrete/bypass rig → reliable 2-device shared rig → single bypass adapter → bench on the default device) until one opens, so it always runs on real audio. ⚠️ **Windows discrete-routing trap (`docs/28`):** the OS shared-mode mixer **down-mixes a multichannel stream onto the device's speaker layout — if the USB DAC is set to *Stereo*, ch3-8 COLLAPSE onto the FRONT jack.** Fix: set **both USB Sound Devices to "7.1 Surround"** in Windows Sound settings (then shared-mode routes discretely), or use a bypass path (WDM-KS — but this CM6206 driver won't open two simultaneous KS streams). **macOS CoreAudio routes discretely with no fold — the judged machine is unaffected.** The engine loudly warns when it lands on a folding shared-mode path. Validated on the real 2-Vantec rig: 66-pair sweep (32 cross-Vantec) reliably drives both Vantecs + per-channel routing assertion.
...
- **Volume pots: bypassed — set level per channel in software** (`docs/18` Exp. 4). You can't exceed the chip's fixed gain; punch comes from coupling + drive frequency, not overdrive (clipping cooks the coil). **Implemented:** the engine reads per-channel software gain from [`config/encoding.json`](config/encoding.json) (`channel_gain`, all 1.0 until the on-body Exp.4 fills real per-site values) and enforces an always-on **clip ceiling 0.95** so no channel ever reaches digital full-scale. (`docs/28`.)
...
### 3.8 Sensing
- **Camera: the MacBook front camera** — the same one the live AR interface uses, in the same playing position → **train/serve match**. → MediaPipe + **markerless fretboard homography**. A front view foreshortens the neck and the hand self-occludes the contact point (that's the occlusion problem the model solves); the homography still recovers the fretboard plane and labels are prompt-grounded, so a noisier `d` doesn't break training. **Registration is MARKERLESS (resolved):** fit the fretboard homography from the guitar's own geometry via the 12-TET fret-spacing law (`x_n = 1−2^(−n/12)` — the instrument is its own ruler), validated by reprojecting the frets onto the real wires. Code: `software/ai/vision/fretboard.py`; capture a per-guitar **digital twin** (reference keyframes at the playing angles) with `calibrate.html`/`twin.py`. A **printed ArUco marker is OPTIONAL** — a ground-truth validation rig only (`cad/aruco/`), and if used it goes **coplanar with the fretboard near the nut/neck, never the angled headstock**. Data-collection protocol: `docs/24`; offline processing + registration contract: `docs/25`. **Framing = the WHOLE guitar in frame** (full-screen — a low "nut→fret 7" angle made the screen unusable, and the AR demo + train/serve match both want the full view), captured at **1080p**; the offline pipeline **crops to the fretboard ROI** so wide framing keeps full `d` resolution (`docs/25 §6d`). Frets **1–6** stay the only rendered/coached scope (6 motors); the camera just sees more (which *helps* registration).
...
- **Output engine (BUILT — `software/haptic/`, `docs/28`):** this renderer sits on the deterministic per-speaker control API. `HapticEngine.play(ch, intensity|amp, freq_hz, duration_ms, waveform=tone|glide, attack_ms, release_ms, delay_ms)` is the full basic control surface exposed to the ML/encoder (non-blocking, overlapping voices across both Vantecs). The §5 *behaviours* above (sweep/bloom/shimmer) are composed by issuing staggered `play()` calls (`delay_ms`). **Assumption (new, tunable):** intensity levels {1,2,3} → amplitude {0.30, 0.60, 0.90} (channel_map declares 3 levels but not the values), defaults attack 4 ms / release 12 ms; all live in [`config/encoding.json`](config/encoding.json), to be re-tuned on-body (`docs/18`).
...
- **Stage 1 — WHERE (vision-led):** MediaPipe + **markerless fretboard homography** (fret-law; ArUco optional) → fretboard-relative pose → `(string, fret, finger, d=fingertip-to-wire)`; deterministic when visible, a **trained occlusion model** when the hand hides the contact point. **Prompt = the label; audio only verifies** (audio cannot give fret).
...
2. **ALSA bring-up** — bind Vantecs by-id, verify in-card channel order, fill `channel_map.json`. (§3.2) *Helper shipped:* the haptic engine auto-resolves output via an **ordered fallback** — discrete bypass rig → reliable 2-device shared rig → single bypass adapter → bench — validating each candidate **non-destructively** and opening the first that works (`docs/28`); `speaker_check.py --sweep` still confirms the physical jack↔channel order to write back into `channel_map.json`. **Measured 2026-06-21 (reconciled after the engine fix):** with the corrected engine **both Vantecs open simultaneously on shared-mode DirectSound** → `mode=rig`, both boxes driven, all 12 channels routed (66-pair sweep, 32 cross-Vantec, runs clean every launch). The earlier report that *the 2nd stream fails on every API → collapse to one box* was an artifact of a **failed WDM-KS open poisoning the shared-mode fallback**; fixed by **non-destructive device discovery** (no probe-open of the flaky KS pin — that probe was destabilizing it) + **skipping the 2-device WDM-KS rig**. The genuine WDM-KS limit remains: this CM6206 driver **won't open two simultaneous *exclusive* KS streams** (both adapters share one USB controller; the 2nd isochronous OUT endpoint can't allocate), so the discrete-bypass path on Windows is **single-Vantec only (8ch)**. **The real remaining collapse is the shared-mode FOLD:** the devices are set to *Stereo*, so DirectSound down-mixes 8ch→2ch onto each box's FRONT jack. **Fix: set BOTH USB Sound Devices to "7.1 Surround" in Windows Sound** (then DirectSound routes discretely — verify by the engine's fold HEADS-UP warning disappearing), or run on the **Mac (CoreAudio = bypass, no fold)**, or two separate USB controllers for the exclusive KS path. *Helper:* `software/haptic/low_note_all.py` fires all 12 at once and **refuses (exit 2) unless `--force`** unless it can reach both boxes discretely (now correctly trips the FOLD GUARD on the DirectSound rig, not the concentration guard).
