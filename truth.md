...
2. **ALSA bring-up** — bind Vantecs by-id, verify in-card channel order, fill `channel_map.json`. (§3.2) *Helper shipped:* the haptic engine auto-resolves the two adapters by real open-test with host-API fallback + bench mode (`docs/28`); `speaker_check.py --sweep` still confirms the physical jack↔channel order to write back into `channel_map.json`. On the dev box WDM-KS validated but failed to open (`-9996`) → the engine used DirectSound; resolve the mixer-bypass path (or use Linux `by-id`) for discrete routing on the judged machine.
