...
- **The UI is [`web/glow.html`](web/glow.html)** (serve `web/`; `/` redirects to it). The live fret-glow LEARN view: per-string 6-hue markers annotated **on the real neck** over the webcam (Sherbet theme), ADSR glow envelope, and alphaTab real tab on the left synced **silent** (render-only, no synth — correct for a Deaf-focused app) from one `SONG` source that also feeds the glow. **The camera is always on** and the grid maps onto the real neck two ways (the 12-TET law lays the frets inside the 4-corner quad): **(1) AUTO** — `web/hand-anchor.js` (MediaPipe Hands) anchors the grid to the fretting hand (robust to busy backgrounds; approximate placement); **(2) MANUAL fallback** — drag the 4 corner dots onto the neck once, saved to localStorage (exact, bulletproof on any background). A "✋ auto-fit / ✥ drag mode" toggle switches between them. NOTE: the earlier OpenCV/Hough edge detector (`web/fretboard-autodetect.js`, port of `software/ai/vision/fretboard_detect.py`) was **confirmed to fail on real busy backgrounds** (curtains + banners out-line the thin diagonal neck → zero neck edges) and is no longer loaded by the UI; it stays in-repo for clean-background/offline use. `web/aruco-poselock.js` (js-aruco2 marker) remains an optional robustness path. Earlier prototypes are archived in `web/_prototypes/`, theme drafts in `web/_themes/` — **not the product**.
