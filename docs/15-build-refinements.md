...
| Old SK473 present? | If only 6 amps → **12 channels**: drop the 2 reserved back zones (z7/z8), keep 6 strings + 6 forearm fret-zones. No re-wire of the rest. Design accommodates this. |
...
- **Count correction: 6 boxes × 2 ch = 12 channels** (not 24, not 14). Map → **6 chest strings + 6 forearm fret-zones**, drop the 2 reserved back zones (this is exactly the §8 fallback). Update `config/channel_map.json` (remove ch 13–14) + the body map. *(If a 7th prior SK473 board exists and opens, that's +2 = 14.)* The **24× 40 mm speakers stay our body actuators** — 12 used, 12 spare.
...
- **Channel count stays 12** (6 boards × 2). Map → 6 chest strings + 6 forearm fret-zones.
