...
- **Registration (markerless-primary):** fit the **fretboard homography from the guitar's own geometry** (12-TET fret-law + the per-guitar digital twin, `software/ai/vision/fretboard.py`) → per-frame neck pose, **temporally smoothed** (one-euro / Kalman) to remove jitter. Notes are rendered in the **neck coordinate frame** and projected through the pose, so they stick to the real frets as the neck moves. An **ArUco marker is an OPTIONAL fallback** if live markerless pose is too jittery (place it coplanar near the nut/neck, not the angled headstock); the **virtual-neck fallback below is the real demo insurance** regardless.
